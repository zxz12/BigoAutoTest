...
class HTMLTestRunner(TemplateMixing):
    ...
    def _generate_report(self, result):
        ...
        for cid, (cls, cls_results) in enumerate(sorted_result):
            ...
            name="%s.py"%(cls.__name__)  # modified by:zxz 20210406
            # if cls.__module__=="__main__":
                # name=cls.__name__
            # else:
                # name="%s.%s"%(cls.__module__, cls.__name__)
            doc=cls.__doc__ or ""
            # # added by:zxz 20210406
            # print(doc)
            try:
                doc=re.search(r'@Description:(.*)', doc).group(1)
                # print(doc)
            except:
                doc=''
            # print(doc)
...
class SMTP(object):
    ...
    def sender(self, to=None, subject=None, contents=None, path=None):
        if to is None:
        ...
        if subject is None:
            subject='Test Report'
        if contents is None:
            total=RunResult.passed+RunResult.failed+RunResult.errors+RunResult.skiped
            if total!=0:
                pass_rate='{:.2%}'.format(RunResult.passed/total)
            else:
                pass_rate='{:.2%}'.format(RunResult.passed/1)
            import pathlib
            href=pathlib.Path(path).as_uri()
            contents=env.get_template('email_temp.html').render(
                mail_href=href,
                mail_pass_rate=pass_rate,
